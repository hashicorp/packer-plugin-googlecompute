...
type Config struct {
	...
	// the instance will be deleted. Defaults to `DELETE` when max_run_duration_in_seconds is specified.
	// Please see [GCE Limit VM Runtime](https://cloud.google.com/compute/docs/instances/limit-vm-runtime)
...
func (c *Config) Prepare(raws ...interface{}) ([]string, error) {
	...
	if c.MaxRunDurationInSeconds > 0 && c.InstanceTerminationAction == "" {
		warnings = append(warnings,
			"max_run_duration is set, but instance_termination_action is not set. Defaulting to 'DELETE'.")
		// If max_run_duration is set, we default to DELETE.
		c.InstanceTerminationAction = "DELETE"
	}
