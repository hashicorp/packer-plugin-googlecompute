...
func (c *Config) Prepare(raws ...interface{}) ([]string, error) {
	...
	if c.MachineImageName == "" {
		img, err := interpolate.Render("packer-machine-image-{{timestamp}}", nil)
		if err != nil {
			errs = packersdk.MultiErrorAppend(errs,
				fmt.Errorf("Unable to parse machine image name: %s ", err))
		} else {
			c.MachineImageName = img
		}
	}
