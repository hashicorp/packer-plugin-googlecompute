...
func (d *Datasource) Execute() (cty.Value, error) {
	...
	filterExpr := buildFilterExpr(d.config.Filters)
	images, err := service.Images.List(d.config.ProjectID).Filter(filterExpr).Do()
	if err != nil {
		return cty.NullVal(cty.EmptyObject), err
	}

	if len(images.Items) == 0 {
		return cty.NullVal(cty.EmptyObject), fmt.Errorf("no images found with filter expression: %q", filterExpr)
	}
